...
class W {
  ...
  #initializePositions() {
    ...
    for (let i = 1; i < config.count; i++) {
      ...
      positionData[idx + 1] = MathUtils.randFloat(config.maxY, 3 * config.maxY);
  ...
  update(deltaInfo: { delta: number }) {
    ...
    for (let idx = startIdx; idx < config.count; idx++) {
      ...
      if (config.controlSphere0) {
        const cursorPos = new Vector3().fromArray(positionData, 0);
        const toCursor  = new Vector3().copy(cursorPos).sub(pos); // ball -> cursor
        const d0 = toCursor.length();
        const sumR0 = radius + sizeData[0];

        if (d0 < sumR0) {
          // positional separation
          const penetration = (sumR0 - d0);
          const toward      = toCursor.normalize();
          const away        = toward.clone().multiplyScalar(penetration);

          pos.sub(away); // push out

          // strong impulse away from cursor (short, punchy)
          const k = (config.pointerForce ?? 20) * 3.0; // bump this to taste
          vel.sub(toward.multiplyScalar(k));           // add impulse away
        }
      }
...
const XConfig = {
  count: 200,
  colors: [0x000000, 0x2C2C2C, 0x39FF14, 0x00FF7F, 0x228B22, 0x76FF7A],
...
function createBallpit(
  ...
  config.maxX = threeInstance.size.wWidth / 2;
  config.maxY = threeInstance.size.wHeight / 2;
